...
class WordificationDriver():
    ...
    def build_root(self):
        ...
        self.login_box = Canvas(self.root)
        self.username_label = "Username:"
        self.password_label = "Password:"

        self.menu_items['labels'] = []

        text = "Welcome to Wordification!"
        text2 = "Please login to continue"
        self.menu_items['labels'].append(Label(self.login_box, text=text, font = self.head_font))
        self.menu_items['labels'][-1].grid(row = 0, column = 0, columnspan = 4)

        self.menu_items['labels'].append(Label(self.login_box, text=text2, font = self.med_font))
        self.menu_items['labels'][-1].grid(row = 1, column = 0, columnspan = 3)

        self.menu_items['labels'].append(Label(self.login_box, text=self.username_label, font = self.med_font))
        self.menu_items['labels'][-1].grid(row = 2, column = 0)
        self.username = Entry(self.login_box, width=15, font = self.med_font)
        self.username.grid(row = 2, column = 1)

        self.menu_items['labels'].append(Label(self.login_box, text=self.password_label, font = self.med_font))
        self.menu_items['labels'][-1].grid(row =3, column = 0)
        self.password = Entry(self.login_box, show="*", width=15, font = self.med_font)
        self.password.grid(row = 3, column = 1)

        self.menu_items["login"] = Button(self.login_box, text='    Login    ', \
        command=self.call_login, font = self.med_font)
        self.menu_items['login'].grid(row = 4, column = 0, columnspan = 3)

        self.login_box.pack()


    def call_login(self):
        client_response = try_login(self.client, self.username.get(), self.password.get(), self.DEBUG)
        print("Client responded with %s" %client_response)
        student_level = 0
        try:
            """
            client_response = client_response.split(",")
            level = int(float(client_response[0]))
            exercise = client_response[1]
            has_tested_today = client_response[2]
            """
        ...
        self.render_home(exercise, category)
        """
    ...
    """
    def render_home(self, exercise, category):
        ...
        if category == "Test":
            ...
            text_b = " Learn! "
        self.menu_items["exercise"] = Button(self.home_box, text = text_b, font = self.med_font, command=lambda data = [exercise, category]: self.call_game_method(data))
        self.menu_items["exercise"].grid(row =2 , column = 0, columnspan = 3)
        self.home_box.pack()

    def re_render_home(self, exercise, category):
        self.root = Tk()
        self.menu_items = defaultdict()
        self.root.title('Wordification® - Main Menu')
        license_text = "Wordification®/SpellingBits®"
    ...
    def call_game_method(self, data):
        ...
        _category = data[1]
        self.root.destroy()
        if _exercise == "Sorting":
            game = sorting.Game(_category, self.student.level)
        elif _exercise == "Matching":    
            game = matching.Game(_category, self.student.level)
        ...
        if score["passed"] and score["exercise"] == "test":
            self.re_render_home("Teach", _category)
